...
export interface PanelHandle {
  /**
   * Asks the panel to perform a navigation action (close, switch episode…).
   * If the form has unsaved changes, the panel shows a confirmation dialog
   * first. If not, the action is executed immediately.
   */
...
export const EpisodeDetailPanel = forwardRef<PanelHandle, Props>(
  function EpisodeDetailPanel(
    ...
    function discardAndContinue() {
      ...
      // Reset state so dirty=false next time
      setForm(initial);
      action?.();
    }
        ...
        >
          <DialogContent className="border-foreground/10 sm:max-w-md">
            <DialogHeader>
              ...
              <DialogDescription>
                Tu as des changements sur «{" "}
                <strong className="text-foreground">{episode.name}</strong> » qui
                ne sont pas encore sauvegardés. Que veux-tu faire ?
              </DialogDescription>
            </DialogHeader>
